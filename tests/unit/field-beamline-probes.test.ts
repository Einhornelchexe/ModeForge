...
test("S16-R6 unresolvable focus raises an explicit warning; pre-focus probe stays silent", () => {
  const run = (probes: number[]) => {
    const job = runHeadlessJob({
      kind: "field-beamline",
      input: {
        beamline: {
          ...
          components: [
            { id: "d", kind: "free-space", lengthMm: 50 },
            // real LA1116-like f=10mm singlet: focus w ~ 6.7 um at z ~ 60.8
            { id: "L", kind: "thick-lens", radius1Mm: 5.15, radius2Mm: "Infinity", thicknessMm: 2.46, refractiveIndex: 1.5168, apertureRadiusMm: 3 },
          ],
        },
        grid: { nx: 128, ny: 128, dxMm: 0.0235, dyMm: 0.0235 },
        probesZmm: probes,
      },
    });
    if (!job.ok || job.value.kind !== "field-beamline") throw new Error("expected field-beamline result");
    return job.value.result.warnings;
  };
  const atFocus = run([60.81]);
  assert.ok(
    atFocus.some((w) => w.code === "FIELD_SAMPLING_LOW" && w.severity === "warning" && w.message.includes("CANNOT be resolved")),
    "probing at an unresolvable focus must warn explicitly",
  );
  const beforeFocus = run([52.46]);
  assert.ok(!beforeFocus.some((w) => w.message.includes("CANNOT be resolved")), "walk that stays away from the focus must not warn");
});
