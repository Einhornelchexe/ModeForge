...
function renderBeamlineMode(T: Strings): string {
  ...
  if (probeOut && dense && dense.zGridMm.length > 0 && probeOut.zMm <= (dense.zGridMm.at(-1) ?? 0) + 1e-9) {
    ...
    const color = hasAperture ? "#6FA8F5" : worst < 0.02 ? "#5CE1A0" : "#F2B33D";
    const note = hasAperture ? T.analyticNoteAp : worst < 0.02 ? T.analyticNoteOk : T.analyticNoteBad;
...
function renderSourceMode(T: Strings): string {
  ...
  if (res) {
    try {
      const wA = gaussianRadiusAtZ(Number(f.dist), Number(f.waist), Number(f.lambda));
