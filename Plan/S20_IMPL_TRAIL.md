...
## S23 — Betreiber-Feedback-Runde nach 2.1.0 (v2.1.1)

**Status: abgeschlossen.** Vier Punkte aus Patricks Live-Test plus ein Bug-Report, umgesetzt im neuen Kanal-Muster (Implementierungs-Kanal baut, UI-Kanal half beim Delta, Review = ein Maximum-Effort-Leg des Review-Kanals): (1) **Warnungs-Lokalisierung** — 41 Warncodes tragen jetzt zweisprachige Beschreibungen; nach Review-Befund als ZUSATZZEILE zur Engine-Meldung (die Rohmeldung mit ihren Zahlen bleibt gedimmt sichtbar — parameterisierte Codes hätten sonst ihre Information verloren). (2) **Richtwert-Erklärung** als Info-Box (Hover/Fokus öffnet die ausführliche Begründung; faktisch korrigiert: Profil- bzw. Fit-Breiten, alle sieben Freigabe-Gates benannt). (3) **Neue INFO `IMAGE_BEAM_IN_BACKGROUND_REFERENCE`** — feuert, wenn eine benutzte Hintergrund-Referenz die 4σ-Ellipse des Strahls schneidet (exakter Segment/Ellipse-Schnitt nach Review-Härtung statt Kanten-Sampling; θ-Rotations-Orakel π/4 beidseitig; Absence-Digests unverändert). (4) **Hintergrund-Rechtecke ohne Zeichenmodus greifbar** — Treffer-Priorität ROI-Griffkante → Nutzer-Referenzrechteck → ROI-Fläche; Auto-Ecken bleiben display-only; Idle-Handles sichtbar. Rotations-ROI-Frage fachlich beantwortet (achsenparalleles Rechenfenster + hauptachsen-rotierte Messblende; Iterations-Box umschließt bereits die gedrehte Ellipse) — bleibt Backlog. Review: 19 akzeptierte Findings (4 HIGH, darunter der Ersetzen-statt-Ergänzen-Fehler der Lokalisierung und eine zunächst undeklarierte Zahlen-Parsing-Änderung — nachträglich deklariert, mit bewusster Leer-Feld-Semantik und Tests). Ein Fix-Lauf wurde extern abgebrochen und per verify-first-Resume sauber zu Ende geführt (16/19 lagen schon auf Disk, 3 vervollständigt). Gates final: 541/541 Unit, beide Typechecks, Wortlaut-Gate, Playwright 16/16, Digests stabil. Version 2.1.1.

**Doc-Version:** 6.0, 2026-08-25.
