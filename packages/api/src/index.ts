...
function runFieldFresnelJob(input: FieldFresnelJobInput): FieldFresnelJobResult {
  let field = input.field ?? (input.gaussian ? createGaussianField(input.gaussian) : undefined);
  ...
  if (beam.kind === "elliptical-gaussian") {
    ...
    return {
      ...
      m2Warning:
        m2x === 1 && m2y === 1
          ? undefined
          : warning("UNSUPPORTED_PROFILE_PROPAGATION", "Field beamline does not exactly model different x/y M2 values.", "warning"),
    };
  }
  return {
    ...
    wavelengthUm: beam.wavelengthUm,
    m2Warning: warning("UNSUPPORTED_PROFILE_PROPAGATION", "Field beamline approximates moment beams as Gaussian fields.", "warning"),
  };
}

function fieldFromBeam(beam: BeamInput, grid: FieldGridInput): { field: ScalarField; warnings: SimulationWarning[] } {
...
function runFieldBeamlineJob(input: FieldBeamlineJobInput): FieldBeamlineJobResult {
  ...
  const { field: initialField, warnings } = fieldFromBeam(input.beamline.beam, input.grid);
  ...
  return {
    ...
    probes,
    image: finalImage,
    warnings: [...analytical.warnings, ...warnings, ...surfaceWarnings, ...probeWarnings, ...samplingWarnings, ...probeSamplingWarnings],
  };
}
